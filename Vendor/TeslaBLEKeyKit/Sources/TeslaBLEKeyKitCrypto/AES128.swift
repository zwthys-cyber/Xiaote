import Foundation
#if !COCOAPODS
import TeslaBLEKeyKitCore
#endif

public struct AES128: Sendable {
    private static let sBox: [UInt8] = [
        ,
    ]
    
    private static let rcon: [UInt8] = [
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
    ]
    
    private let roundKeys: [UInt8]
    
    public init(key: Data) throws {
        guard key.count == 16 else {
            throw TeslaError.crypto("AES-128 requires a 16-byte key")
        }
        self.roundKeys = Self.expandKey(Array(key))
    }
    
    public func encryptBlock(_ input: [UInt8]) throws -> [UInt8] {
        guard input.count == 16 else {
            throw TeslaError.crypto("AES block must be 16 bytes")
        }
        var state = input
        addRoundKey(&state, round: 0)
        for round in 1..<10 {
            subBytes(&state)
            shiftRows(&state)
            mixColumns(&state)
            addRoundKey(&state, round: round)
        }
        subBytes(&state)
        shiftRows(&state)
        addRoundKey(&state, round: 10)
        return state
    }
    
    private static func expandKey(_ key: [UInt8]) -> [UInt8] {
        var expanded = Array(repeating: UInt8(0), count: 176)
        for index in 0..<16 {
            expanded[index] = key[index]
        }
        
        var bytesGenerated = 16
        var rconIndex = 1
        var temp = Array(repeating: UInt8(0), count: 4)
        
        while bytesGenerated < 176 {
            for index in 0..<4 {
                temp[index] = expanded[bytesGenerated - 4 + index]
            }
            if bytesGenerated % 16 == 0 {
                temp = [temp[1], temp[2], temp[3], temp[0]]
                for index in 0..<4 {
                    temp[index] = sBox[Int(temp[index])]
                }
                temp[0] ^= rcon[rconIndex]
                rconIndex += 1
            }
            for index in 0..<4 {
                expanded[bytesGenerated] = expanded[bytesGenerated - 16] ^ temp[index]
                bytesGenerated += 1
            }
        }
        return expanded
    }
    
    private func addRoundKey(_ state: inout [UInt8], round: Int) {
        let offset = round * 16
        for index in 0..<16 {
            state[index] ^= roundKeys[offset + index]
        }
    }
    
    private func subBytes(_ state: inout [UInt8]) {
        for index in 0..<16 {
            state[index] = Self.sBox[Int(state[index])]
        }
    }
    
    private func shiftRows(_ state: inout [UInt8]) {
        state = [
            state[0], state[5], state[10], state[15],
            state[4], state[9], state[14], state[3],
            state[8], state[13], state[2], state[7],
            state[12], state[1], state[6], state[11],
        ]
    }
    
    private func mixColumns(_ state: inout [UInt8]) {
        for column in 0..<4 {
            let offset = column * 4
            let a0 = state[offset]
            let a1 = state[offset + 1]
            let a2 = state[offset + 2]
            let a3 = state[offset + 3]
            
            state[offset] = multiplyBy2(a0) ^ multiplyBy3(a1) ^ a2 ^ a3
            state[offset + 1] = a0 ^ multiplyBy2(a1) ^ multiplyBy3(a2) ^ a3
            state[offset + 2] = a0 ^ a1 ^ multiplyBy2(a2) ^ multiplyBy3(a3)
            state[offset + 3] = multiplyBy3(a0) ^ a1 ^ a2 ^ multiplyBy2(a3)
        }
    }
    
    private func multiplyBy2(_ value: UInt8) -> UInt8 {
        let shifted = value << 1
        return (value & 0x80) == 0 ? shifted : shifted ^ 0x1b
    }
    
    private func multiplyBy3(_ value: UInt8) -> UInt8 {
        multiplyBy2(value) ^ value
    }
}
