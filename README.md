...
密钥使用 `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`，手机锁屏时仍可响应拉门认证，但不会同步或迁移到其他设备。清理 Keychain 或换机后需要重新配对；卸载 App 前应先在 App 内忘记车辆，并在车机“控制 > 锁”中删除旧钥匙记录。被动钥匙依赖 App 在后台运行：不要从多任务界面划掉 App，重启手机后先打开一次 App 再靠近车辆。
